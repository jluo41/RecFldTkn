...
* `config_recfldtkn`: The folder contains the raw data input information.

* `fn_recfld`: tools to convert the raw record table into clean and oragnized table. Eg. `Rx` means convert the `prescription_xxx.csv` into `Rx` table with clean `IDs`, `DTs`, and `attribute/field columns`. 

* `fn_fldtkn`: tools to generate the feature sets for each record. Eg. `Rx-InsCateTkn`, this means the function to convert a `Rx` record into a token list of `InsCateTkn`. 
...
## 1.1 Split Egm Table
```shell
python run_cohort_preprocess_drfirst.py --cohort_label 1 --egm_filename "engagement_df_de_identified.csv"
```

## 1.2 Run Cohort Information
```shell
python run_cohort_info_drfirst.py --cohort_label 1
```

# 2. Process Record Field
...
# 4. Convert RecFldTkn to Huggingface Datasets

```shell
python ../scripts/run_recfldtkn_to_hfds.py  --record_name "P"
python ../scripts/run_recfldtkn_to_hfds.py  --record_name "PInv"
python ../scripts/run_recfldtkn_to_hfds.py  --record_name "Rx" 
python ../scripts/run_recfldtkn_to_hfds.py  --record_name "EgmClick"
python ../scripts/run_recfldtkn_to_hfds.py  --record_name "EgmAuthen"
python ../scripts/run_recfldtkn_to_hfds.py  --record_name "EgmCallPharm"
python ../scripts/run_recfldtkn_to_hfds.py  --record_name "EgmCopay"
python ../scripts/run_recfldtkn_to_hfds.py  --record_name "EgmEdu"
python ../scripts/run_recfldtkn_to_hfds.py  --record_name "EgmRmd"
```
...
```shell
# python bash_rft_drfirst.py --step1 --step2 --step3 --step4 --core_num 4
python bash_rft_drfirst.py --step1 --core_num 1
python bash_rft_drfirst.py --step2 --core_num 1
python bash_rft_drfirst.py --step3 --core_num 1
python bash_rft_drfirst.py --step4 --core_num 4
```