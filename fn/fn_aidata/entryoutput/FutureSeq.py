...
def transform_fn_output(examples, tfm_fn_AIInputData, OneEntryArgs, CF_to_CFvocab):
    ...
    future_start = Output_Part['FutureStart']
    future_period = Output_Part['FuturePeriod']
    future_end = future_start + future_period

    input_ids = examples_tfm['input_ids']
    input_ids_new = input_ids[:, :future_start]
    labels = input_ids[:, future_start:future_end]

    examples_tfm['input_ids'] = input_ids_new
    examples_tfm['labels'] = labels
    # if 'hm_ids' in examples_tfm:
    #     hm_ids = examples_tfm['hm_ids']
    #     hm_ids_new = hm_ids[:, :future_start]
    #     examples_tfm['hm_ids'] = hm_ids_new
